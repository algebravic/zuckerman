...
numbers in a number of sequences:
...
The program given in [Smallest number that is n times the product of its digits or 0 if impossible. ](https://oeis.org/A056770 "Smallest") implicitly uses this
...
digits. This, however, is extremely wasteful. We would only like to
consider all multisets of digits of size $k$ so that $m$ times their
product is in the interval $[10^{k-1}, 10^k - 1]$. We give an
efficient backtrack program which accomplishes this. In addition, it
appears that the number of such grows only linearly with $m$.

The idea in the backtrack program, is to contruct each sorted sequence
from the largest to the smallest element. We recursively use an
interval into which the product of the digits must fit. After choosing
the largest element (and its multiplicity) we then calculate a smaller
interval that the remaining product must fit in.
